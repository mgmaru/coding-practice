...
func innerJoinById(users []UserInfo, orders []OrderInfo) []UserOrderInfo {
	...
	userOrderInfo := make([]UserOrderInfo, 0, len(users)) // 出力スライス
	userIdItemList := make(map[int][]string, len(orders)) // 一次マップ
	...
	for _, user := range users { // usersはスライスなので並びも保証
		for _, item := range userIdItemList[user.id] { // useridのitemリストが１個以上あればappend。空の場合は何もしない。
