...
- 15_top_n_test.go実装
#### わかったこと
- テスト駆動開発の練習として、テストから書いたが、これがこれで開発しやすい気がした。
- 最初、`map`のまま処理しようとしていたが、ソートする際に頻度にアクセスできない問題が発生。
  - 構造体で定義して、頻度をフィールド`Frequency`にすることで、アクセスしやすいようにした。
#### 反省
- `map`と構造体が似ている気がする。どのタイミングで`map`を使用するのか、構造体を利用するのかを明確にする。
- `map`を使用する場合と構造体を使用する場合のメリットデメリットを調べて明確にする。
#### 実装時間
- 初期実装：1時間41分
---
### 2026/7/22
...
### 2026/7/23
### 総合反省
- 基本的なコレクションを一通り学ぶことができた。
- Goの基本的なテストの書き方を学ぶことができた。
- 最後にもあったが、Goの組み込み関数をもっと使用できれば、効率的にコードをかけたかもしれない。
- 数学チックなアルゴリズム系の問題に時間がかかっているので、基本的なアルゴリズムを一通り学んだほうが良い。
- とはいえ、コーディングの習慣がつき、コーディングの敷居が低くなった気がするので、今後も1日1時間から2時間で取り組める範囲のコードを毎日書いていきたい。
